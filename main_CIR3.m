...
for  i = 1:trainIDX01
    X_train_tmp01(:,1,1,i) = mat2gray(abs(Dyn_re_CIR01(i+num_tst01,:))');
    X_train_tmp01(:,2,1,i) = mat2gray(abs(Bg_re_CIR01)');
end
for  i = 1:trainIDX02
    X_train_tmp02(:,1,1,i) = mat2gray(abs(Dyn_re_CIR02(i+num_tst02,:))');
    X_train_tmp02(:,2,1,i) = mat2gray(abs(Bg_re_CIR02)');
end
for  i = 1:trainIDX04
    X_train_tmp04(:,1,1,i) = mat2gray(abs(Dyn_re_CIR04(i+num_tst04,:))');
    X_train_tmp04(:,2,1,i) = mat2gray(abs(Bg_re_CIR04)');
end
for  i = 1:trainIDX12
    X_train_tmp12(:,1,1,i) = mat2gray(abs(Dyn_re_CIR12(i+num_tst12,:))');
    X_train_tmp12(:,2,1,i) = mat2gray(abs(Bg_re_CIR12)');
end
for  i = 1:trainIDX14
    X_train_tmp14(:,1,1,i) = mat2gray(abs(Dyn_re_CIR14(i+num_tst14,:))');
    X_train_tmp14(:,2,1,i) = mat2gray(abs(Bg_re_CIR14)');
end
for  i = 1:trainIDX24
    X_train_tmp24(:,1,1,i) = mat2gray(abs(Dyn_re_CIR24(i+num_tst24,:))');
...
if ~isfile(['ConvNet_CIR3.mat'])
    ConvNet_CIR3 = CIR_CNN_CIRVar_Tst(X_train,Y_train,X_val,Y_val,"CIR");
    save ConvNet_CIR3.mat ConvNet_CIR3
else
    load(['ConvNet_CIR3.mat']);
end

%% test
for i = 1:num_tst01
    X_test01(:,1,1,i) = mat2gray(abs(Dyn_re_CIR01(i,:))');
    X_test01(:,2,1,i) = mat2gray(abs(Bg_re_CIR01)');
end
for i = 1:num_tst02
    X_test02(:,1,1,i) = mat2gray(abs(Dyn_re_CIR02(i,:))');
    X_test02(:,2,1,i) = mat2gray(abs(Bg_re_CIR02)');
end
for i = 1:num_tst04
    X_test04(:,1,1,i) = mat2gray(abs(Dyn_re_CIR04(i,:))');
    X_test04(:,2,1,i) = mat2gray(abs(Bg_re_CIR04)');
end
for i = 1:num_tst12
    X_test12(:,1,1,i) = mat2gray(abs(Dyn_re_CIR12(i,:))');
    X_test12(:,2,1,i) = mat2gray(abs(Bg_re_CIR12)');
end
for i = 1:num_tst14
    X_test14(:,1,1,i) = mat2gray(abs(Dyn_re_CIR14(i,:))');
    X_test14(:,2,1,i) = mat2gray(abs(Bg_re_CIR14)');
end
for i = 1:num_tst24
    X_test24(:,1,1,i) = mat2gray(abs(Dyn_re_CIR24(i,:))');
...
Y_pred01 = predict(ConvNet_CIR3,X_test01);
Y_pred02 = predict(ConvNet_CIR3,X_test02);
Y_pred04 = predict(ConvNet_CIR3,X_test04);
Y_pred12 = predict(ConvNet_CIR3,X_test12);
Y_pred14 = predict(ConvNet_CIR3,X_test14);
Y_pred24 = predict(ConvNet_CIR3,X_test24);
...
Y_pred_val_tmp = predict(ConvNet_CIR3,X_val);
